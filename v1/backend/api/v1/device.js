...
const authorize = (req, res, next) => {
    ...
    token.checkRequest((req.headers.authorization).split(' ')[1])
        .then(async (result) => {
            ...
            if(await devices.findUser({_id: mogodb.ObjectId(req.id), disabled: true})) return res.status(404).json(result(404, 'Your device has been disabled'));
...
app.get('/me/disable', authorize, async (req, res) => {
    ...
    await devices.disable(req.id, true);
