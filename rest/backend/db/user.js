...
const createDevice = async (usermodel) => {
    return new Promise((resolve, rejects) => {
        ...
        try {
            ...
            db().collection('mobile').insertOne(model, (err, res) => {
                if(err) rejects(err);
                return resolve(res)
            })
        } catch (error) {
...
const deviceExists = async (id) => {
    return new Promise(async (resolve, rejects) => {
        ...
        const device = await db().collection('mobile').findOne({'_id': mogodb.ObjectId(id)});
...
const getProfile = async (id) => {
    return new Promise(async (resolve, rejects) => {
        ...
        const user = await db().collection('mobile').findOne({'_id': mogodb.ObjectId(id)});
